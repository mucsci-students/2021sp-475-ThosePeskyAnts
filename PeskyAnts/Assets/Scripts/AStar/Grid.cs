...
public class Grid : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        CreateGrid ();
        // InvokeRepeating ("CreateGrid", 1.0f, 2.0f);
    }
    ...
    public void CreateGrid ()
    {
        ...
        for (int i = 0; i < gridSizeX; ++i)
        {
            for (int j = 0; j < gridSizeY; ++j)
            {
                ...
                bool walkable = !(Physics.CheckSphere(worldPoint, nodeRadius + nodePadding, unwalkableMask));
    ...
    public List<Node> GetNeighbors (Node node)
    {
        ...
        for (int x = -1; x <= 1; ++x)
        {
            for (int y = -1; y <= 1; ++y)
            {
                // ignore center node - thats the current node
                if (x == 0 && y == 0)
                    continue; 
